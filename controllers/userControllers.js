...
exports.homePage = async (req, res) => {
  try {
    ...
    // console.log(products)
    res.render("user/men", { products, user, cartCount, categories });
  } catch (error) {
    console.log(error);
  }
};
...
exports.getWomenPage = async (req, res) => {
  ...
  res.render("user/women", { products, user, cartCount });
};

exports.getKidsPage = async (req, res) => {
  const products = await Product.find({ type: "Baby's & Kid's" , deleted: false });
  let user = req.session.user;
  // Access cartCount value from req object
  const cartCount = req.cartCount;

  res.render("user/kids", { products, user, cartCount });
};
